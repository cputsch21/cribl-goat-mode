...
export function PersonaRehearsal({ persona }: { persona: Persona }) {
  return (
    <ConversationProvider>
      <RehearsalInner persona={persona} />
    </ConversationProvider>
  );
}

function RehearsalInner({ persona }: { persona: Persona }) {
  const [phase, setPhase] = useState<Phase>("idle");
  const [errorMsg, setErrorMsg] = useState("");
  const [transcript, setTranscript] = useState<DojoSimLine[]>([]);
  ...
  const conversation = useConversation({
    ...
    onMessage: (m: { message?: string; source?: string }) => {
      ...
      if (!text) return;
      const role: DojoSimLine["role"] = m?.source === "user" ? "rep" : "buyer";
  ...
  // End the call if the panel unmounts (e.g. navigating away mid-rehearsal).
  useEffect(() => {
    return () => {
  ...
  async function start() {
    ...
    try {
      const res = await fetch("/api/persona-sim", {
        method: "POST",
  ...
  const finish = useCallback(
    async (hangUp: boolean) => {
      ...
      try {
        const res = await fetch("/api/persona-debrief", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ personaId: p.id, transcript: lines, seconds }),
        });
        const data = await res.json();
        ...
        const v = data as DojoDebrief;
        setDebrief(v);
        recordSim(p.id, { seconds, transcript: lines, debrief: v });
  ...
  const result = usePersonaResult(persona.id);
  const last = result?.sims?.[0] ?? null;
        ...
        <div className="rounded-2.5xl bg-surface p-5 shadow-card">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-display text-lg font-bold text-ink">
              Rehearse against {who}
            </h4>
          </div>
          <p className="mt-1.5 text-sm leading-relaxed text-muted">
            Practice out loud against an AI buyer built from this persona. You&apos;ll be
            live with the{" "}
            <span className="font-semibold text-ink">{persona.title.toLowerCase()}</span>.
          ...
          {last?.debrief && phase !== "error" ? (
            <div className="mt-5 border-t border-line pt-5">
              <DebriefView
                debrief={last.debrief}
          ...
          {captionsOn ? (
            <div className="mt-4 max-h-36 w-full space-y-1.5 overflow-y-auto">
              {transcript.slice(-4).map((t, i) => (
                ...
                >
                  <b>{t.role === "buyer" ? who : "You"}:</b> {t.text}
                </p>
              ))}
            </div>
          ) : null}
      ...
      {phase === "tooShort" ? (
        <div className="rounded-2.5xl bg-surface p-8 text-center shadow-card">
          ...
          <p className="mx-auto mt-1 max-w-sm text-sm leading-relaxed text-muted">
            Give the buyer a few real answers before wrapping up, then I can score it.
          </p>
          <button
            onClick={start}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-2.5xl bg-ink p-6 text-center">
        ...
        <p className="mt-1 text-xs text-white/55">execution · ~{mins} min</p>
        <p className="mt-3 text-sm leading-relaxed text-white/80">{debrief.summary}</p>
        <div
          className={cn(
        ...
        >
          ...
          {debrief.advanced ? "Earned the next step" : "Didn't earn it"}
        </div>
